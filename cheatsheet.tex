...
$\mu_{f_{n+1} \rightarrow z_{n+1}}(z_{n+1}) = \underset{z_n}{\text{max}} \{ \text{ln }f_{n+1}(z_n,z_{n+1}) + \mu_{z_n \rightarrow f_{n+1}}(z_n) \}$ \\
Eliminate $\mu_{z_n \rightarrow f_{n+1}}(z_n)$ and use $f_n(z_{n-1},z_n) = p(z_n|z_{n-1})p(x_n|z_n)$ to get a recursion:\\
$\omega(z_{n+1}) = \text{ln } p(x_{n+1}|z_{n+1}) + \underset{z_n}{\text{max }} \{ \text{ln }p(z_{n+1}|z_n) + \omega(z_n)\}$\\
...
\subsubsection{Gibbs sampling formula}
$p(z_{di}=k|w,z^{(-di)}) = \frac{p(z_{di}=k, w, z^{(-di)})}{p(w,z^{(-di)}}$\\
$= \frac{p(z_{di}=k, z^{(-di)})}{p(z^{(-di)})} \times \frac{p(w|z)}{p(q^{(-di)}|z^{(-di)})p(w_{di})}$\\
$\varpropto \frac{\Gamma(A_{dk}+\alpha)}{\Gamma(\sum^K_{j=1} A_{dj}+\alpha)} \times \frac{\Gamma(\sum^K_{j=1}A_{dj}^{(-di)} + \alpha}{\Gamma(A_{dk}^{(-di)} + \alpha)}$\\
$\times \frac{\Gamma(B_{kw}+\beta)}{\Gamma(\sum_w B_{kw} + \beta)} \times \frac{\Gamma(\sum_w B_{kw}^{(-di)} + \beta)}{\Gamma(B_{kw}^{(-di)} +\beta)}$\\
Using $\Gamma(x+1) = x\Gamma(x)$, we write:\\
$p(z_{di}=k|w,z^{-i}) = \frac{A_{dk}^{-i}+\alpha}{\sum^K_{j=1} A_{dk}^{-i} + \alpha} \times \frac{B_{kw}^{-i}+\beta}{\sum_{w\in V} B_{kw}^{-i} + \beta}$\\
$ = \frac{A_{dk}^{-i}+\alpha}{\sum^K_{j=1}A_{dk}^{-i}+\alpha} \times \frac{B^{-i}_{kw} + \beta}{\sum_{w\in V} B_{kw}^{-i} + \beta}$
