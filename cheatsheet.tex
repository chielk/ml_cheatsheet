...
\textbf{Detailed balance}:(or reversibility) We want to find T so that transition from one state to another has probability mass equal to the transition from that next state to the current.\\ Proof:$A_k(z',z) = \text{min }\big( 1, \frac{p(z')q(z|z')}{p(z)q(z'|z)} \big)$\\
